    ...
    case 'sphere':
      return (
        <>
          ...
          <AttributeSlider label="Latitude Bands" min={3} max={20} step={1} value={geometry.latitudeBands} onChange={(val) => onChange({ ...geometry, latitudeBands: val })} />
          <AttributeSlider label="Longitude Bands" min={3} max={20} step={1} value={geometry.longitudeBands} onChange={(val) => onChange({ ...geometry, longitudeBands: val })} />
        </>
      );
    case 'pyramid':
    case 'prism':
      return (
        <>
          <AttributeSlider label="Edges" min={3} max={20} step={1} value={geometry.edges} onChange={(val) => onChange({ ...geometry, edges: val })} />
          <AttributeSlider label="Radius" min={0.125} max={1} value={geometry.radius} onChange={(val) => onChange({ ...geometry, radius: val })} />
          <AttributeSlider label="Height" min={0.25} max={2} value={geometry.height} onChange={(val) => onChange({ ...geometry, height: val })} />
        </>
      );
